...
# Save the LDAP encrypted password to a temporary file
encrypted_password_file = '/tmp/encrypt_pass'
execute "Encrypting password" do
  command "#{node[:console][:install][:path]}/dsutil/bin/crypt.sh #{node[:ldap][:password]} > #{encrypted_password_file}"
  action :run
end
...
#  not_if { ::File.exist?("#{node[:console][:install][:path]}/ldap") }
#end
link node[:console][:dbDIR] do
  to node[:console][:config_path]
...
bash "ldap commands" do
  code <<-EOH
   ...
   ldappasswd -x -D "cn=bluldap,dc=blustratus,dc=com" \
    ...
    ldap_encrypted_password=$(cat #{encrypted_password_file})
...
# Remove the LDAP encrypted password temporary file
file encrypted_password_file do
  backup false
  action :delete
end
