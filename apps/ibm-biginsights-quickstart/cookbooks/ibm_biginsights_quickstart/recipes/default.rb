unless File.exists? "/opt/ibm/biginsights/conf/biginsights.properties"
  ...
  imcloud_client 'BigInsights Quick Start Edition 2.1 FP1'

  bash "extract-biginsights" do
    code lazy { "tar --index-file /tmp/biginsights.tar.log -xvvf " + node[:imcloud_client][:return] + " -C /mnt" }
    action :nothing
  end
  ...
  template "/tmp/install.xml" do
    ...
    variables(
      ...
      :master_hostname => node[:biginsights][:hostname],
    ...
    notifies :run, "bash[extract-biginsights]", :immediately
    notifies :run, "bash[install-biginsights]", :immediately
  end  

  file "Delete BigInsights media" do
    backup false
