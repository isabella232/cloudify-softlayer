...
when "debian", "ubuntu"
  ...
  %w{libstdc++6 lib32stdc++6 libaio1 libpam0g:i386 numactl}.each do |pkg|
    package pkg
  end
...
if ::File.directory?(node[:db2][:install_path])
  ...
  [node[:db2][:data_path], node[:db2][:backup_path], node[:db2][:log_path], node[:db2][:home_path]].each do |dir|
    directory dir do
      mode 00777
      action :create
      recursive true
  ...
  imcloud_client node[:db2][:download_name]
