...
# TODO: DB2 1.5.0.3 s131217 is supposed to fix the libcrypt link issue.
# Validate and remove 'openssl-devel' if so!
...
bash "Update DB2 BM AUTHENTICATION" do
  code <<-EOH
	su - #{node[:db2][:instance][:username]} -c "db2 update dbm cfg using AUTHENTICATION SERVER_ENCRYPT"
    EOH
end
