...
if node[:cognos][:install_fixpacks] == "YES"
  node[:cognos][:fixpack_versions].each do |fp_version|
    name = "cognos_#{fp_version.downcase.gsub(' ', '_')}"
    setup_cognos(name, "Cognos BI #{node[:cognos][:version]} #{fp_version}")
  end
end
